...
<html>
	<head>
		...
		<script>
			...
			function remover(id){
				location.href = `todas_tarefas.php?acao=remover&id=${id}`;//Direcionando a pagina para essa URL, para trabalharmos com o PHP
			}
								...
								<? foreach($tarefasRecuperadas as $indice => $tarefa) { ?>
									<div class="row mb-3 d-flex align-items-center tarefa">
										<div id='tarefa_<?= $tarefa['id'] ?>' class="col-sm-9"><!--Colocando um ID individual para cada tarefa-->
											<?=($tarefa['tarefa'])?> (<?=$tarefa['status']?>)<!--Listando a tarefa e o Status da tarefa-->
										</div>
